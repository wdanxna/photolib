...
//    [self setupEditor];
    ...
    [_finalImageView setFrame:CGRectMake(0, 0, self.view.bounds.size.width, self.view.bounds.size.height-70)];
    ...
    dispatch_async(dispatch_get_global_queue(DISPATCH_QUEUE_PRIORITY_HIGH, 0), ^{
        ...
        if (currentlySelected != 1)
        {
            ...
            if (currentlySelected == 2)
            {
                ...
                cv::GaussianBlur(original, original, cvSize(9,9), 0);
                cv::adaptiveThreshold(original, original, 255, cv::ADAPTIVE_THRESH_GAUSSIAN_C, cv::THRESH_BINARY, 5, 2);
